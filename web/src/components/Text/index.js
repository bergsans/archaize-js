...
<h4 className="essay__container__sub-header" style={{marginTop: '30px'}}>Question</h4>
...
identify <em>differences</em>  in syntax between ES6  and ES5 and
...
also used Vims -d flag (<code>vim -d file1.ast file2.asti</code>).
</p>
...
<li>differences in way  that would produce the  same end result,
identical output.</li>
...
As  Tim   Berners-Lee,the  inventor  of  the   Web,  <a href="https://www.w3.org/People/Berners-Lee/1996/ppf.html)1996" title="">stated
it</a> <quote>information  must be  available on all platforms, including 
future ones</quote>.
</p>
...
<p className="essay__paragraph" >
A    quick     glance    at    <a href="https://kangax.github.io/compat-table/es6/">The     ES6    Compatability
Table</a>   clearly
...
<a href="https://babeljs.io/" title="">Babel.js</a>,  poly-fills  is  (probably)
...
the compiler  doesn't use  the <code>private</code> keyword  this would
...
the  project <a href="https://github.com/estree/estree/" title="">ESTree</a>,  a
...
an arrow function making use of <code>this</code>. If this is the case,
...
gets  assigned to  <code>undefined</code>. Pre-ES6,  you would  set the
...
<p className="essay__paragraph" >
Can we  i.e. say that <code>addEventListener</code>,  a browser method,
is a part  of JavaScript, <code>CreateEvent</code> in  Node.js are part
...
standardize JavaScript features.  It suggests <strong>conventions</strong>,
...
the reference. Thus doing it compatible with ES5: <code>a: a, b: b</code>.
</p>
...
method (<code>defineProperty</code>).  But in a  local scope it  can't be
done.
</p>
...
<p className="essay__paragraph" >
But  if you  would replace  <code>===</code> with  <code>==</code> it  would print
'FALSE'.  Because JavaScript  is  a  dynamically typed  it's
possible, if  the variable declaration did  not use <code>const</code>,
...
other programming languages: <code>boolean</code>, <code>null</code>, <code>undefined</code>,
<code>strings</code>,  <code>numbers</code>  and  as  of  ES6  also  <code>symbols</code>.  A
...
<p className="essay__paragraph" >
With ES6 it's possible to use  <code>let</code> and <code>const</code>, as well as
the pre-ES6 <code>var</code> to  declare values. The difference resides
in  what scope  the variable  and its  inherent value  have.
Also,  there  is a  difference  between  <code>let</code> and  <code>const</code>,
...
<p className="essay__paragraph" >
<code>let</code> and <code>const</code>  have another scope than  <code>var</code>, or rather
can have. This means that the  blocks above would not be the
same - or rather, that  their 'meaning' would be another. In
the example  below the  <code>var</code> would actually  be <code>available</code>
...
that there are obvious advantages  to the <code>let</code> keyword; you
can *see*  the limits  for a declared  value right  away (or
rather, more easily). One could,  however, make some kind of
case for using <code>var</code> for  global values, even though this is
also possible with <code>let</code> and <code>const</code>.
</p>
...
special precautions when copying an  <code>Object</code>, you will end up
...
described  above, we  could replace  <code>let</code> and  <code>const</code> with
<code>var</code>. The purpose with <code>let</code>  and <code>const</code> would (of course)
be lost, but that's not  true for the opposite. To translate
from <code>var</code> to <code>let</code> and fully  make use of the advantages of
<code>let</code>  would be  complex; you  would  have to  make a  scope
...
name declared with <code>let</code> or <code>const</code> (i.e. for the index of a
classic for-loop). Simply converting  each <code>let</code> and <code>const</code>
...
(The    solution    is    from    the    Babel.js    <a href="https://babeljs.io/" title="">Online
transpiler</a> as of 2019-01-22.)
</p>

<p className="essay__paragraph" >
To  me,  the   purpose  of  the  limited   scoping  and  the
immutability  of  <code>const</code>(1)  <em>is</em>  the  limits,  that  they
establish  limits lacking  in JavaScript.  This is  also the
case with TypeScript. Sometimes borders and limits make your
life  as  a developer  simpler,  even  though you  <em>can</em>  do
without them.
</p>
...
accomplished  with   the  <code>freeze</code>  method  of   <code>Object</code>.  It
is,  however,  possible  to  the  same  in  ES5  by  use  of
the  <code>defineProperty</code>   in  <code>Object.prototype</code>.   Setting  the
...
<ul>
<li><em>Commander.js</em>. CLI-functionality (Node.js). </li>
<li><em>Dandy UI</em>. Minimalistic terminal UI library (Node.js). </li>
<li><em>Esprima</em>. Provides the Abstract Syntax Tree. </li>
<li><em>Estraverse</em>. Library for traversing the JSON-like data. </li>
<li><em>Estracode</em>. Transforms a Abstract Syntax Tree to source code.</li> 
</ul>
...
<ul>
<li>By  using the  Archaize JS  NPM package  and by  using the
functionality of <code>makeAST</code> and <code>transpile</code>.</li>


<li>Or by including the CLI in package.json; something I would
recommend.</li>
</ul>
